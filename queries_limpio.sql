...
FROM bebedor b
JOIN gusta g ON g.ci = b.ci
JOIN bebida beb ON beb.codbeb = g.codbeb
WHERE beb.nombrebeb = 'Guepardex'
AND b.ci NOT IN (
    SELECT 
        b.ci
    FROM bebedor b
...
JOIN (
    SELECT
        g.ci,
        f.codfs,
        COUNT(g.codbeb) AS "bebidasQueVendeYLeGustan"
    FROM gusta g
    JOIN frecuenta f ON f.ci = g.ci
    JOIN vende v ON v.codfs = f.codfs AND g.codbeb = v.codbeb
    GROUP BY g.ci, f.codfs
) q1 ON q1.ci = b.ci  AND q0."bebidasQueGusta" = q1."bebidasQueVendeYLeGustan"
JOIN (
    SELECT
        q1.ci,
        COUNT(q1.codfs) AS "fuentesDeSodaQueFrecuenta"
    FROM q1 q1
    GROUP BY q1.ci
) q2 ON q2.ci = b.ci
JOIN (
...
WHERE q2."fuentesDeSodaQueFrecuenta" = q3."count";
...
JOIN (
    ...
    HAVING COUNT(f.ci) = (
        SELECT MAX(CountCI)
        FROM (
            SELECT COUNT(f.ci) AS CountCI
            FROM frecuenta f
            GROUP BY f.codfs
        ) subquery
    )
) q0 ON q0."codfs" = v.codfs
WHERE b.codbeb IN (
    SELECT
        MIN(g.codbeb)
    FROM gusta g
